...
```bash
pnpm-mature update [package...] [-a|--age <days>] [-g|--use-pnpm-global-config] [-r [level]] [-d|--dry-run] [-y|--yes] [--max-registry-mib <mib>]
```
...
```bash
pnpm-mature install [package...] [-a|--age <days>] [-g|--use-pnpm-global-config] [-r [level]] [-d|--dry-run] [--max-registry-mib <mib>]
```
